...
public class NetworkSimulator {
    ...
    public static final int FROM_LAYER2 = 0;
    public static final int LINK_CHANGE = 1;
    ...
    public NetworkSimulator(boolean hasLinkChange, int trace, long seed) {
        ...
        cost = new int[NUM_ENTITIES][NUM_ENTITIES];
        cost[0][0] = 0;
        cost[0][1] = 1;
        cost[0][2] = 3;
        cost[0][3] = 7;
        ...
        entity = new Entity[NUM_ENTITIES];
        ...
        if (linkChanges) {
            eventList.add(new Event(10000.0, LINK_CHANGE, 0));
        }
        ...
        while (true) {
            ...
            if (traceLevel > 1) {
                ...
                if (next.getType() == FROM_LAYER2) {
                    ...
                    System.out.println(p.getMinCost(NUM_ENTITIES - 1) + "]");
                } else if (next.getType() == LINK_CHANGE) {
            ...
            if (next.getType() == FROM_LAYER2) {
                p = next.getPacket();
                if ((next.getEntity() < 0) || (next.getEntity() >= NUM_ENTITIES)) {
                ...
                if (time < 10001.0) {
                    System.out.println("Link 0 - 1 has changed to 20");
                    cost[0][1] = 20;
                    cost[1][0] = 20;
                    entity[0].linkCostChangeHandler(1, 20);
                    entity[1].linkCostChangeHandler(0, 20);
                } else {
                    System.out.println("Link 0 - 1 has changed to 1");
                    cost[0][1] = 1;
                    cost[1][0] = 1;
    ...
    public static void toLayer2(Packet p) {
        ...
        if ((p.getSource() < 0) || (p.getSource() >= NUM_ENTITIES)) {
            System.out.println("toLayer2(): WARNING: Illegal source id in " +
                    "packet; ignoring.");
            return;
        }
        if ((p.getDest() < 0) || (p.getDest() >= NUM_ENTITIES)) {
        ...
        eventList.add(new Event(arrivalTime, FROM_LAYER2,
